...
MODEL = "/home/lixingjian/models/bloom-7b1"
...
def check_inference_engine(use_engine=False, do_sample=False, use_cuda_kernel=False, prompt_template=None, policy=None):
    setup_seed(20)
    tokenizer = BloomTokenizerFast.from_pretrained(MODEL, use_fast=False, trust_remote_code=True)
    model = BloomForCausalLM.from_pretrained(MODEL, trust_remote_code=True).half().cuda()
    model = model.eval()

    inputs = [
        "Introduce some landmarks in France.",
    ]
...
@parameterize("use_cuda_kernel", [False])
def check_tp_engine(prompt_template, do_sample, use_cuda_kernel):
    ...
    for s1, s3 in zip(
        colossal_tp_1_output,
        #   colossal_tp_2_output,
        transformer_tp_1_output,
    ):
